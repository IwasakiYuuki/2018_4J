...
void task2(void)
{
	...
	while (1) {
		D_INT();  /*このデモでは通信中は割り込み禁止にしてリソースの競合を避けている*/
		SCI1_printf("task2 %4d\n",cnt);
		E_INT();
		msecwait(20);
		cnt++;
		if (cnt==10100) cnt=10000;
	}
}
/***************　２つのタスクの実体　ここまで　********************/
...
RETURNEDFROMTASK2:
    ...
    mov.w r0, @_tasksw   ;次のタスク番号を変数taskswに保存
    ;スタックポインタをコンテキスト１のBer7に保存
    mov.l  er7,  @_context2; (context1とcontext1.Ber7は同じアドレス)
    ;コンテキスト0のBer7をスタックポインタにセット
